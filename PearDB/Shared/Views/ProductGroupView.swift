        ...
        HStack {
            VStack {
                ...
                if let lastDevice = devices?.last, let images = lastDevice.images {
                    ForEach(images.index) { image in
                        URLImage(URL(string:"https://img.appledb.dev/device@main/\(lastDevice.key)/\(image.id).webp")!)
                    }
            ...
            VStack(alignment: .leading) {
                if let devices = devices {
                    ...
                    let socs = Array(Set(devices.compactMap { $0.soc } ))
                    if !socs.isEmpty {
                            Text("SoC: \(socs.joined(separator: ", "))").font(.footnote)
                    }
                    Text("Model: \(devices.flatMap { $0.model }.joined(separator: ", "))").font(.footnote)
                    
                    let allReleasedDates = devices.compactMap({ $0.released?.compactMap { $0 } }).flatMap({ $0 })
                    if !allReleasedDates.isEmpty {
